...
const DisplayBooks = (data) => {
  ...
  docs?.forEach((books) => {
    ...
    <img src="https://covers.openlibrary.org/b/id/${
      books.cover_i ? books.cover_i : "sorry image not find"
    }-M.jpg" class="card-img-top h-75 img-fluid" alt="sorry image not found sir">
    
    <div class="card-body">
      <h5 class="card-title">${books.title ? books.title : "cant find"}</h5>
      <p class="card-text">authors name:<span class ="text-info">${
        books.author_name ? books.author_name[0] : "sorry not find"
      }</span></p>
     
    <P> published year: <span class = "text-info" > ${
      books.first_publish_year ? books.first_publish_year : "cant find!"
    }</span></P>
    </div>
  </div>
